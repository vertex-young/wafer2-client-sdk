...
function request(options) {
  ...
  function doRequestWithLogin() {
    var loginOpts = { success: doRequest, fail: callFail };
    if (options.method) {
      utils.extend(loginOpts, { method: options.method });
    }
    if (options.data) {
      utils.extend(loginOpts, { data: options.data });
    }
    if (options.header) {
      utils.extend(loginOpts, { header: options.header });
    }
    loginLib.login(loginOpts);
  }
